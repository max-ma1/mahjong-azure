...
class friendmodel {
	...
	function get_list($uid, $page, $pagesize, $totalnum, $direction = 0) {
		...
		if($sqladd) {
            //$sql = "SELECT f.*, m.username FROM ".UC_DBTABLEPRE."friends f LEFT JOIN ".UC_DBTABLEPRE."members m ON f.friendid=m.uid WHERE $sqladd LIMIT $start, $pagesize";
            $sql = "SELECT * FROM " . UC_DBNAME. ".`pre_home_friend` WHERE `uid` = '{$uid}'";
