...
/**
 * 通过 Google 提供的 API 验证“人机身份验证”是否通过
 *
 * @param  string $user_response 客户端/用户产生的“输入结果”
 *
 * @return bool 通过验证返回true，否则false
 */
function checkGOOGLEreCAPTCHA($user_response = '')
{
	$api_uri = 'https://www.google.com/recaptcha/api/siteverify';

	$request_params = array(
		'secret'   => '6LfPrP8SAAAAAKielRMtB4-qBIRwT_WaWmqS8HMd',
		'response' => trim($user_response),
		'remoteip' => getIpAddressForGOOGLEreCAPTCHA()
	);

	$request_uri = $api_uri . '?' .  http_build_query($request_params);

	//CURL 在调用 SSL 的链接时存在安全证书认证的问题，方便起见还是用 file_get_contents() 为好。
	//关于安全证书的问题，可以参考 <http://stackoverflow.com/questions/6400300/https-and-ssl3-get-server-certificatecertificate-verify-failed-ca-is-ok>
...
function getIpAddressForGOOGLEreCAPTCHA() {
	$ipaddress = '';
